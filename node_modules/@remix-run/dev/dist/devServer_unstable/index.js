...
let serve = async (initialConfig, options) => {
  ...
  let startAppServer = command => {
    ...
    let newAppServer = execa__default["default"].command(cmd, {
      ...
      logger.logger.error("app failed to start" + colors__default["default"].gray(` (${command})`));
      throw e;
    });
  ...
  let dispose = await watch.watch({
    ...
    onBuildFinish: async (ctx, durationMs, succeeded) => {
      if (!succeeded) return;
      let msg = (state.prevManifest ? "rebuilt" : "built") + colors__default["default"].gray(` (${prettyMs__default["default"](durationMs)})`);
      websocket.log(msg);
      logger.logger.info(msg);
      ...
      try {
        ...
        if (state.prevManifest) {
          logger.logger.info(`app server ready` + colors__default["default"].gray(` (${prettyMs__default["default"](Date.now() - start)})`));
        }
    ...
    onFileCreated: file => {
      logger.logger.info(`rebuilding...` + colors__default["default"].gray(` (+ ${relativePath(file)})`));
