...
async function resolveViteConfig({
  ...
  let viteConfig = await vite$1.resolveConfig({
    mode,
    configFile,
    root
  }, "build",
...
const resolveRelativeRouteFilePath = (route, remixConfig) => {
  ...
  return vite$1.normalizePath(fullPath);
};
...
const resolveChunk = (ctx, viteManifest, absoluteFilePath) => {
  let vite$1 = vite.getVite();
  let rootRelativeFilePath = vite$1.normalizePath(path__namespace.relative(ctx.rootDirectory, absoluteFilePath));
...
const remixVitePlugin = (remixUserConfig = {}) => {
  ...
  return [{
    ...
    config: async (_viteUserConfig, _viteConfigEnv) => {
      ...
      Object.assign(process.env, vite$1.loadEnv(viteConfigEnv.mode, ctx.rootDirectory,
    ...
    async configResolved(resolvedViteConfig) {
      ...
      let vite$1 = vite.getVite();
      let childCompilerConfigFile = await vite$1.loadConfigFromFile({
      ...
      viteChildCompiler = await vite$1.createServer({
        ...viteUserConfig,
        mode: viteConfig.mode,
        server: {
    ...
    async configureServer(viteDevServer) {
      ...
      viteDevServer.watcher.on("all", async (eventName, rawFilepath) => {
        ...
        let appFileAddedOrRemoved = (eventName === "add" || eventName === "unlink") && filepath.startsWith(normalizePath(ctx.remixConfig.appDirectory));
    ...
    writeBundle: {
      ...
      async handler() {
        ...
        if (movedAssetPaths.length) {
          viteConfig.logger.info(["", `${colors__default["default"].green("✓")} ${movedAssetPaths.length} asset${movedAssetPaths.length > 1 ? "s" : ""} moved from Remix server build to client assets.`, ...movedAssetPaths.map(movedAssetPath => colors__default["default"].dim(path__namespace.relative(ctx.rootDirectory, movedAssetPath))), ""].join("\n"));
        }
    ...
    async buildEnd() {
      var _viteChildCompiler, _routesViteNodeContex7;
      await ((_viteChildCompiler = viteChildCompiler) === null || _viteChildCompiler === void 0 ? void 0 : _viteChildCompiler.close());
      await ((_routesViteNodeContex7 = routesViteNodeContext) === null || _routesViteNodeContex7 === void 0 ? void 0 : _routesViteNodeContex7.devServer.close());
    }
  }, {
    ...
    async resolveId(id, importer, options) {
      ...
      let vite$1 = vite.getVite();
      let importerShort = vite$1.normalizePath(path__namespace.relative(ctx.rootDirectory, importer));
    ...
    hotUpdate({
      ...
      let clientModules = uniqueNodes(modules.flatMap(mod => getParentClientNodes(server.environments.client.moduleGraph, mod)));
...
function getParentClientNodes(clientModuleGraph, module) {
  ...
  return [...module.importers].flatMap(importer => getParentClientNodes(clientModuleGraph, importer));
}
...
function getRoute(pluginConfig, file) {
  let vite$1 = vite.getVite();
  let routePath = vite$1.normalizePath(path__namespace.relative(pluginConfig.appDirectory, file));
  let route = Object.values(pluginConfig.routes).find(r => vite$1.normalizePath(r.file) === routePath);
  return route;
}
