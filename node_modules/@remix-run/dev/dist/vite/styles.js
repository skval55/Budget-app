...
const injectQuery = (url, query) => url.includes("?") ? url.replace("?", `?${query}&`) : `${url}?${query}`;
const getStylesForFiles = async ({
  ...
  let viteMajor = parseInt(vite$1.version.split(".")[0], 10);
  let styles = {};
  let deps = new Set();
  try {
  ...
  for (let dep of deps) {
    if (dep.file && isCssFile(dep.file) && !isCssUrlWithoutSideEffects(dep.url) // Ignore styles that resolved as URLs, inline or raw. These shouldn't get injected.
      ...
      try {
        ...
        viteMajor >= 6 ? injectQuery(dep.url, "inline") : dep.url)).default;
        if (css === undefined) {
          throw new Error();
        }
        styles[dep.url] = css;
